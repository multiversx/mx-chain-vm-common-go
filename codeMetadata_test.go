...
func TestCodeMetadata_FromBytes(t *testing.T) {
	...
	require.True(t, CodeMetadataFromBytes([]byte{8, 0}).Frozen)
	require.False(t, CodeMetadataFromBytes([]byte{0, 8}).Frozen)
	require.False(t, CodeMetadataFromBytes([]byte{4, 0}).Frozen)
	require.False(t, CodeMetadataFromBytes([]byte{1, 0}).Frozen)
}
...
func TestCodeMetadata_ToBytes(t *testing.T) {
	...
	require.Equal(t, byte(8), (&CodeMetadata{Frozen: true}).ToBytes()[0])
}
