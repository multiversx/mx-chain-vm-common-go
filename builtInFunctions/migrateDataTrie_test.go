...
func TestMigrateDataTrie_ProcessBuiltinFunction(t *testing.T) {
	...
	t.Run("dataTrieMigrator err", func(t *testing.T) {
		...
		mdtf, _ := NewMigrateDataTrieFunc(vmcommon.BuiltInCost{TrieLoad: 110}, &mock.EnableEpochsHandlerStub{}, &mock.AccountsStub{})
		vmOutput, err := mdtf.ProcessBuiltinFunction(mock.NewUserAccount([]byte("sender")), mock.NewUserAccount([]byte("dest")), input)
		assert.Nil(t, vmOutput)
	...
	t.Run("saves dest account", func(t *testing.T) {
		...
		mdtf, _ := NewMigrateDataTrieFunc(vmcommon.BuiltInCost{TrieLoad: 50}, &mock.EnableEpochsHandlerStub{}, accStub)
...
func TestMigrateDataTrie_SetNewGasConfig(t *testing.T) {
	...
	mdtf, _ := NewMigrateDataTrieFunc(vmcommon.BuiltInCost{TrieLoad: 50}, &mock.EnableEpochsHandlerStub{}, &mock.AccountsStub{})
	mdtf.SetNewGasConfig(&vmcommon.GasCost{BuiltInCost: vmcommon.BuiltInCost{TrieLoad: 100}})
	assert.Equal(t, uint64(100), mdtf.builtInCost.TrieLoad)
}
