...
func (e *esdtNFTMultiTransfer) transferOneTokenOnSenderShard(
	...
	_, err = e.esdtStorageHandler.SaveESDTNFTToken(acntSnd.AddressBytes(), acntSnd, esdtTokenKey, transferData.ESDTTokenNonce, esdtData, isReturnCallWithError)
	if err != nil {
		return nil, err
	}
...
func (e *esdtNFTMultiTransfer) createESDTNFTOutputTransfers(
	...
	for i, esdtTransfer := range listESDTTransfers {
		...
		if esdtTransfer.ESDTTokenNonce > 0 {
			wasAlreadySent, err := e.esdtStorageHandler.WasAlreadySentToDestinationShard(esdtTransfer.ESDTTokenName, esdtTransfer.ESDTTokenNonce, dstAddress)
			if err != nil {
				return err
			}
